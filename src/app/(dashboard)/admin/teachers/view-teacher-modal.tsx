...
export function ViewTeacherModal({ teacher, open, onOpenChange }: ViewTeacherModalProps) {
    if (!teacher) return null;

    const getEmploymentBadge = (type: string) => {
        switch (type) {
            case "FULL_TIME": return <Badge className="bg-blue-500/10 text-blue-600 hover:bg-blue-500/20">Full Time</Badge>;
            case "PART_TIME": return <Badge className="bg-orange-500/10 text-orange-600 hover:bg-orange-500/20">Part Time</Badge>;
            case "CONTRACT": return <Badge className="bg-purple-500/10 text-purple-600 hover:bg-purple-500/20">Contract Base</Badge>;
            case "GUEST": return <Badge className="bg-gray-500/10 text-gray-600 hover:bg-gray-500/20">Guest Lecturer</Badge>;
            ...
            <SheetContent className="sm:max-w-[650px] w-full p-0 flex flex-col h-full bg-slate-50/50 dark:bg-slate-950/50 border-l-0 shadow-2xl">
                ...
                <div className="flex-1 overflow-y-auto p-8">
                    <Tabs defaultValue="overview" className="w-full">
                        <TabsList className="grid w-full grid-cols-3 mb-8 bg-muted/50 p-1 rounded-xl shadow-inner">
