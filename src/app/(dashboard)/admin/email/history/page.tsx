...
export default function EmailHistoryPage() {
    ...
    const { data: response, isLoading } = useQuery({
        queryKey: ["email-transactions", page],
        queryFn: () => EmailService.getEmailTransactions({ page, limit }),
            ...
            <CardHeader className="border-b border-border/40 bg-muted/10 pb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <CardTitle className="text-xl font-extrabold flex items-center gap-2">
                        <History className="h-6 w-6 text-primary" /> Email Accounting Ledger
                    </CardTitle>
                    <CardDescription className="text-sm font-medium mt-1">
                        Track all your email credit top-ups and campaign usage history.
                    </CardDescription>
                </div>
                            ...
                            {isLoading ? (
                                ...
                                transactions.map((tx: any) => {
                                    ...
                                    return (
                                        <TableRow key={tx.id} className="h-20 border-b border-border/40 even:bg-muted/5 hover:bg-muted/20 transition-colors">
                                            ...
                                            <TableCell className="px-6">
                                                <p className="text-sm font-semibold text-foreground max-w-[350px] truncate" title={tx.description}>{tx.description}</p>
                                                {tx.referenceId && (
                                ...
                                <TableRow>
                                    <TableCell colSpan={6} className="h-64 text-center text-muted-foreground">
                                        ...
                                        <p className="text-sm mt-1">Your email top-up and usage history will appear here.</p>
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
