...
import { MessageSquare, Send, Zap, Loader2, History, AlertTriangle, CheckCircle2 } from "lucide-react";
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center h-[50vh] gap-4">
                ...
                <p className="text-sm font-semibold text-muted-foreground">Syncing communication data...</p>
            </div>
        );
    }

    const balance = stats?.availableCredits || 0;
    const isLowBalance = balance < 500; // Trigger warning if credits drop below 500
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {isLowBalance && (
                <div className="flex items-center gap-3 bg-destructive/10 border border-destructive/20 text-destructive p-4 rounded-xl shadow-sm animate-in slide-in-from-top-2">
                    ...
                    <div className="flex-1">
                        <h4 className="text-sm font-extrabold uppercase tracking-wide">Critical: Low SMS Balance</h4>
                    ...
                    <Button asChild size="sm" variant="destructive" className="font-bold shadow-md whitespace-nowrap">
                        <Link href="/admin/sms/recharge">Recharge Now</Link>
                    </Button>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                <Card className="shadow-sm border-border bg-card hover:border-primary/30 transition-all">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2 text-lg"><Send className="h-5 w-5 text-primary" /> New Broadcast</CardTitle>
                        <CardDescription>Send an important notice or alert to students or teachers instantly.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Button asChild className="w-full h-12 font-bold shadow-md" disabled={balance === 0}>
                            <Link href="/admin/sms/compose">Launch Campaign <Send className="ml-2 h-4 w-4" /></Link>
                        </Button>
                    </CardContent>
                </Card>
                ...
                <Card className="shadow-sm border-border bg-card hover:border-border/80 transition-all">
                    <CardHeader>
                        ...
                        <CardDescription>Review your latest SMS dispatch history and recharge invoices.</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Button asChild variant="outline" className="w-full h-12 font-bold shadow-sm">
                            <Link href="/admin/sms/history">View Ledger <History className="ml-2 h-4 w-4" /></Link>
                        </Button>
                    </CardContent>
                </Card>
            </div>
