    ...
    const mutation = useMutation({
        mutationFn: async (values: PackageFormValues) => {
            if (isEditing && initialData) {
                return await EmailService.updatePackage(initialData.id, values);
            }
            return await EmailService.createPackage(values);
        },
        onSuccess: () => {
        ...
        <Sheet open={isOpen} onOpenChange={onClose}>
            <SheetContent className="w-full sm:max-w-[540px] flex flex-col h-full p-0">
                <SheetHeader className="p-6 border-b bg-muted/20">
                    <SheetTitle>{isEditing ? "Edit Email Package" : "Add Email Package"}</SheetTitle>
                    <SheetDescription>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    <Form {...form}>
                        <form id="email-package-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <FormField control={control} name="credits" render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="font-bold">Email Credits</FormLabel>
                                        <FormControl>
                ...
                <div className="p-6 border-t bg-muted/5 flex items-center justify-end gap-3">
                    ...
                    <Button type="submit" form="email-package-form" disabled={mutation.isPending}>
