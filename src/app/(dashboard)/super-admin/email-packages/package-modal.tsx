...
import { Loader2, PackagePlus, Edit3, Mail } from "lucide-react";
...
export function PackageModal({ isOpen, onClose, initialData }: PackageModalProps) {
    ...
    const form = useForm<PackageFormValues>({
        ...
        defaultValues: { name: "", credits: 0, price: 0, isActive: true },
    });

    useEffect(() => {
        if (initialData && isOpen) {
            form.reset({ name: initialData.name, credits: initialData.credits, price: initialData.price, isActive: initialData.isActive });
        } else if (!isOpen) {
    ...
    const onSubmit = async (values: PackageFormValues) => {
        ...
        try {
            if (isEditing && initialData) {
                await EmailService.updatePackage(initialData.id, values);
                toast.success("Email Package updated successfully!");
            } else {
                await EmailService.createPackage(values);
                toast.success("Email Package created successfully!");
            }
            ...
            <DialogContent className="sm:max-w-[450px] max-h-[90vh] overflow-y-auto custom-scrollbar">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2 text-xl font-bold">
                        ...
                        {isEditing ? "Edit Email Package" : "Create Email Package"}
                    </DialogTitle>
                    <DialogDescription>
                        {isEditing ? "Modify the details of this email credit bundle." : "Define a new email credit bundle for schools to purchase."}
                    </DialogDescription>
                </DialogHeader>
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 pt-2">
                        <FormField control={form.control} name="name" render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl><Input placeholder="E.g., Gold Pack - 10k Emails" className="h-11 shadow-sm" {...field} /></FormControl>
                                <FormMessage />
                            </FormItem>
                        )} />
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <FormField control={form.control} name="credits" render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="font-bold flex items-center gap-1"><Mail className="h-3.5 w-3.5 text-primary" /> Email Credits</FormLabel>
                        ...
                        <DialogFooter className="pt-4 border-t border-border/40 sm:justify-center">
                            <div className="grid grid-cols-2 gap-3 w-full">
                                <Button type="button" variant="outline" onClick={onClose} className="w-full h-11 font-bold">Cancel</Button>
