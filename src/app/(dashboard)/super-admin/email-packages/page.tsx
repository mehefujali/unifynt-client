...
export default function EmailPackagesAdminPage() {
    const queryClient = useQueryClient();
    
    const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
    const [editingPackage, setEditingPackage] = useState<EmailPackage | null>(null);
    ...
    const handleOpenEditModal = (pkg: EmailPackage) => {
        setEditingPackage(pkg);
        setIsModalOpen(true);
    };

    const handleToggleStatus = async (pkg: EmailPackage) => {
        try {
            await EmailService.updatePackage(pkg.id, { isActive: !pkg.isActive });
            toast.success(`Package successfully ${!pkg.isActive ? 'activated' : 'paused'}!`);
            queryClient.invalidateQueries({ queryKey: ["email-packages"] });
        } catch {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await EmailService.deletePackage(id);
            ...
            fallback={
                <div className="flex flex-col items-center justify-center py-32 text-center animate-in fade-in zoom-in-95 duration-500">
                    ...
                    <p className="text-muted-foreground mt-2 max-w-md mx-auto">
                        You do not have the clearance level required to manage global Email credit bundles.
                    </p>
                </div>
            }
        >
            <div className="space-y-6 animate-in fade-in duration-500 max-w-[1400px] mx-auto pb-10 px-4 md:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <Card className="shadow-sm border-border bg-card rounded-[24px] overflow-hidden">
                        ...
                        <CardContent className="pt-4"><div className="text-4xl font-black tracking-tighter tabular-nums text-emerald-600">{packages.filter((p: EmailPackage) => p.isActive).length}</div></CardContent>
                    </Card>
                </div>
                            ...
                            <Table>
                                <TableHeader className="bg-slate-50/50 dark:bg-slate-950/30">
                                    <TableRow className="border-none">
                                        ...
                                        <TableHead className="h-14 px-8 font-black text-[11px] uppercase tracking-[2px] text-slate-400">Email Credits</TableHead>
                                        ...
                                        packages.map((pkg: EmailPackage) => (
                                            <TableRow key={pkg.id} className="group h-20 border-b border-black/5 dark:border-white/5 hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-all">
                                                ...
                                                <TableCell className="px-8">
                                                    <Badge variant="outline" className="font-black text-[11px] px-3 py-1 bg-primary/10 text-primary border-primary/20 shadow-none uppercase tracking-wider">
                                                        <Mail className="h-3 w-3 mr-1.5" /> {pkg.credits.toLocaleString()} Credits
                                                    </Badge>
                                                </TableCell>
                                                <TableCell className="px-8">
